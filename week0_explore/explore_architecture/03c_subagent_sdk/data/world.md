...
Persistent memory of places discovered in this MUD, **shared across every
character played here** (`dummy`, `jibbus`, ...) since the map itself
doesn't depend on which character is exploring it. **Read at session start**
and **append/update whenever a new room, exit, NPC, or landmark is found.**
Record each room by its exact name, its exits, and anything notable. Each
character's own level/vitals/inventory/position lives in its own
`data/<character>/player.md` instead — never here.
