...
Port of ``lib/boukensha/config.rb`` from the Ruby ``04_api_client`` step. The Ruby source's
``PROMPTS_DIR`` gained an extra ``../`` in this step (resolving outside the step's own directory
to a nonexistent path) — an apparent off-by-one typo, not ported; this file stays byte-identical
to the correct calculation already used in ``00_config``/``03_prompt_builder``.
"""
