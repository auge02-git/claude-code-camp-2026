...
In this step the library is unchanged in behaviour; what's new lives outside the
package — the standalone ``boukensha_loader`` module and the ``bin/boukensha``
executable that turn it into a global command.
"""
...
def repl(
    *,
    ...
    """Interactive REPL (Ruby's ``Boukensha.repl``).

    Same options as :func:`run`, minus ``task`` — the user supplies tasks
    interactively. Register tools in ``setup``; then the REPL loop takes over,
    accumulating conversation history across turns until the user exits (or
    sends EOF / interrupts).
    """
    cfg = config()  # loads .env; populates os.environ
    task_class = Player
    task_settings = cfg.tasks(task_class.task_name())
